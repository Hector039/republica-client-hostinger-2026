...
const urlMonthlyCLubElectronicInfo = "utils/monthlyclubelectronic/";
const urlMonthlyMonthlyElectronicInfo = "utils/monthlyelectronic/";
const urlMonthlyAnnualElectronicInfo = "utils/monthlyannualelectronic/";
const urlMonthlyRequestsElectronicInfo = "utils/monthlyrequestselectronic/";
const urlMonthlyExpendituresElectronicInfo =
	"utils/monthlyexpenditureselectronic/";
const urlMonthlyIncomeElectronicInfo = "utils/monthlyincomeelectronic/";
const urlInfoElectronic = "utils/getmonthgridinfoelectronic/";

export default function MonthlyElectronic() {
	const today =
	...
	function getClubInfo(month) {
		axios
			.get(urlMonthlyCLubElectronicInfo + month)
			.then((response) => {
	...
	function getMonthlyInfo(month) {
		axios
			.get(urlMonthlyMonthlyElectronicInfo + month)
			.then((response) => {
	...
	function getAnnualInfo(month) {
		axios
			.get(urlMonthlyAnnualElectronicInfo + month)
			.then((response) => {
 ...
 */
	function getRequestsInfo(month) {
		axios
			.get(urlMonthlyRequestsElectronicInfo + month)
			.then((response) => {
	...
	function getExpendituresInfo(month) {
		axios
			.get(urlMonthlyExpendituresElectronicInfo + month)
			.then((response) => {
	...
	function getIncomeInfo(month) {
		axios
			.get(urlMonthlyIncomeElectronicInfo + month)
			.then((response) => {
						...
						<tbody>
							{requestsInfo.map((merch) => (
								<tr key={merch.registros}>
									<th>
										{new Date(merch.pay_date).toLocaleDateString("en-GB", {
											timeZone: "UTC",
										})}
									</th>
									<th>{merch.total}</th>
				...
				:	<table>
						...
						<tbody>
							{expendituresInfo.map((exp) => (
								<tr key={exp.id_exp}>
									<th>
										{new Date(exp.pay_date).toLocaleDateString("en-GB", {
											timeZone: "UTC",
										})}
									</th>
									<th>{exp.descr}</th>
				...
				:	<table>
						...
						<tbody>
							{info.map((grid) => (
								<tr key={grid.id}>
									<th>
										{new Date(grid.day).toLocaleDateString("en-GB", { timeZone: "UTC" })}
									</th>
									<th>{grid.totalIn}</th>
									<th>{grid.totalOut}</th>
									<th>{grid.total}</th>
						...
						<tfoot>
							<tr>
								...
								<th></th>
								<th>
									${info.reduce((acumulador, item) => acumulador + item.total, 0)}
								</th>
							</tr>
						</tfoot>
					</table>
